function [D_all, Sigma_bar_all, X_all, S_all] = simulate_tensor_S_random(I1, I2, I3, c, total_n ,ratio_s, ratio_o, magnitude)
...
Col_ = rand(size(X));
Col_S = double(Col_<=ratioS);
S = double(S_) .* double(Col_S);

% Corresponding column for X is 0
All = tenones(size(X));
Col_X = double(All) - Col_S;
X = double(X) .* double(Col_X);
