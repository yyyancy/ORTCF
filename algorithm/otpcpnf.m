function [X_recov,S] = otpcpnf(M_0,X,rank,iter,lambda_2,lambda_3,lambda_4,lambda_5)
...
for i = 1: iter
    ...
    for n = 1: dim_(2)
        m = M_0(:,n,:,:);
        %更新右张量R和噪声张量S
        [s,r_M,r_N] = R_update(m,D_M,L_N,r_M,r_N,rank,lambda_2,lambda_3,lambda_4,transformH);
        S(:,n,:,:) = s;
        R_M(:,n,:,:) = r_M;
        R_N(:,n,:,:) = r_N;
        ...
        B_M = B_M + H_tprod(m-s-ln,H_tran(r_M,transformH),transformH);
        ...
        B_M = B_M + H_tprod(m-s-lm,H_tran(r_M,transformH),transformH);
...
function [s_,r_M,r_N] = R_update(m_,D_M,L_N,r_M,r_N,rank_,lambda_2,lambda_3,lambda_4,transform)
    d_ = size(m_);
    ...
    converged = false;
    while ~converged
        r_M = H_tprod(DM_tilde,m_-s_- H_tprod(L_N,r_N,transform),transform);
        r_N = H_tprod(LN_tilde,m_-s_- H_tprod(D_M,r_M,transform),transform);
        s_ = soft_threshold(m_ - H_tprod(D_M,r_M,transform)- H_tprod(L_N,r_N,transform),lambda_2);
